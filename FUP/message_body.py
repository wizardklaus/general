...
class BodyRequest(ISerializable): #파일 전송 요청 메시지(0x01)에 사용할 본문 클래스, FILESIZE,FILENAME 데이터 속성을 가짐
...
class BodyResponse(ISerializable): #파일 전송 요청에 대한 응답메시지(0x02)에 사용 요청 메시지의 MSCID와 수락 여부를 나타내는 RESPONSE 속성
    def __init__(self,buffer):
...
class BodyData(ISerializable): #실제 파일을 전송하는 메시지(0x03)에 사용, DATA필드만 갖고 있다
    def __init__(self,buffer):
        if buffer!=None:
            self.DATA=buffer
...
class BodyResult(ISerializable): #파일 전송 결과 메시지(0x04)에 사용 요청 메시지의 MSGID, 성공 여부를 나타내는 RESULT 속성을 가짐
    def __init__(self,buffer):
