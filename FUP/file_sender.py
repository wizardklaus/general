...
if __name__ == '__main__':
    ...
    try:
        print("서버:{0}/{1}".format(serverIp, serverPort)) #접속 요청을 수행한다
        ...
        MessageUtil.send(sock, reqMsg) #클라이언트는 서버에 접속하자마자 파일 전송 요청 메시지를 보낸다
        ...
        with open(filepath, 'rb') as file: #서버에서 전송 요청을 수락했다면, 파일을 열어 서버로 보낼 준비를 한다
            ...
            while totalRead < filesize:
                ...
                fileMsg = Message()
                fileMsg.Body = BodyData(rbytes) #모든 파일의 내용이 전송될 때까지의 파일을 0x03 메시지에 담아 서버로 보낸다
