...
class FileReceiveHandler(socketserver.BaseRequestHandler):
    def handle(self):
        ...
        reqMsg=MessageUtil.receive(client) #클라이언트가 보내온 파일 전송 요청 메시지를 수신합니다.
        ...
        if answer.strip()!='yes': #사용자가 yes가 아닌 답을 입력하면 클라이언트에게 거부응답을 보낸다.
            ...
            MessageUtil.send(client,rspMsg) #yes를 입력하면 클라이언트에게 승낙응답을 보낸다
            print("파일 전송을 시작합니다...")
            ...
            with open(upload_dir + "\\" + fileName, 'wb') as file: #업로드 받을 파일을 생성한다
                ...
                if fileSize == recvFileSize: #파일 전송 요청에 담겨온 파일 크기와 실제로 받은 파일의 크기를 비교하여 같으면 성공 메시지를 보낸다
                    MessageUtil.send(client, rstMsg)
                else:
                    rstMsg.Body = BodyResult(None)
                    rstMsg.Body.MSGID = reqMsg.Header.MSGID
                    rstMsg.Body.RESULT = message.FAIL
                    MessageUtil.send(client, rstMsg) #파일 크기에 이상이 있다면 실패 메시지를 보낸다
        ...
        if __name__ == '__main__':
            ...
            bindPort = 5425 #서버의 포트는 5425
            server = None
            try:
                server = socketserver.TCPServer(
                    ('', bindPort), FileReceiveHandler) #IP 주소를 ''으로 입력하면 클라이언트는 서버 애플리케이션에 할당된 어떤 주소로도 접속이 가능하다
