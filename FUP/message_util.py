...
class MessageUtil:
    @staticmethod
    def send(sock,msg):
        #send() 는 msg 매개변수가 담고 있는 모든 바이트를 내보낼 때까지 반속해서
        #socket.send() 메소드를 호출한다.
        sent=0
        buffer=msg.GetBytes()
    ...
    def receive(sock):
        ...
        #헤더 읽기
        while sizeToRead>0: #첫 반복문에서 스트림으로부터 메시지 헤더의 경계를 끊음
        ...
        while sizeToRead>0: #첫 반복문에서 얻은 헤더에서 본문의 길이를 뽑아내어
            #그 길이만큼 다시 스트림으로부터 본문을 읽는다.
            buffer=sock.recv(sizeToRead)
            if not buffer:
                return None
